...
let interval;
window.maxHeight = 0;
let currentproyect = 0;

const copyEmail = (e) => {
  e.preventDefault();
  const btnSendEmail = e.target;
  const email = document.querySelector("#email-to-copy");
  ...
  if (navigator.clipboard) {
    navigator.clipboard
      ...
      .then(() => {
        clipboardContainer.style.backgroundColor = "#6eff6e";
        setTimeout(() => {
          clipboardContainer.style.backgroundColor = "#919191";
        }, 1000);
      })
      .catch(() => {
        clipboardContainer.style.backgroundColor = "#ff6d6d";
        setTimeout(() => {
          clipboardContainer.style.backgroundColor = "#919191";
        }, 1000);
      });
...
const proyectCardContainer = document.querySelector(".proyects2");
const details = document.querySelectorAll(".card-details2");
for (let idx = 0; idx < details.length; idx++) {
  details[idx].classList.add("ocultar");
  details[idx].addEventListener("mouseleave", () => {
    details[idx].classList.remove("ver");
    details[idx].classList.add("ocultar");
  });
  proyectCardContainer.addEventListener("mouseenter", () => {
    details[idx].classList.remove("ocultar");
    details[idx].classList.add("ver");
  });
}
// details.classList.add("ocultar");
// details.addEventListener("mouseleave", () => {
//   details.classList.remove("ver");
//   details.classList.add("ocultar");
//   proyectCardContainer.style.boxShadow =
//     "0 0 500px 1px rgba(203, 200, 214, 0.5)";
// });
// proyectCardContainer.addEventListener("mouseenter", () => {
//   details.classList.remove("ocultar");
//   details.classList.add("ver");
//   proyectCardContainer.style.boxShadow =
//     "0 0 10px 1px rgba(203, 200, 214, 0.7)";
// });

const next = () => {
  container = document.querySelector(".full");
  wContainer = container.clientWidth;
  if (currentproyect === container.children.length - 1) {
    currentproyect = 0;
  } else {
    currentproyect++;
  }
  container.style.transform = `translateX(${-wContainer * currentproyect}px)`;
  for (let idx = 0; idx < details.length; idx++) {
    details[idx].classList.remove("ver");
...
const prev = () => {
  ...
  wContainer = container.clientWidth;
  if (currentproyect === 0) {
    currentproyect = container.children.length - 1;
  } else {
    currentproyect--;
  }
  container.style.transform = `translateX(${-wContainer * currentproyect}px)`;
...
const sendMail = () => {
  ...
  btn.addEventListener("click", async (e) => {
    ...
    const isSend = await emailjs.sendForm(
      "service_apqe69l",
      "template_541lwes",
      form,
      "yiovq8u8C6DBGB2UR"
...
window.addEventListener("load", function () {
  ...
  window.maxHeight = `${
    Array.from(proyects).reduce(
      (prev, value) => (value.offsetHeight > prev ? value.offsetHeight : prev),
      0
    ) + 10000
  }px`;
